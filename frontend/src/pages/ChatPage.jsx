...
import { Send, Bot, User, Sparkles, FileText, Target, CreditCard, ShieldAlert } from 'lucide-react';
  ...
  const [messages, setMessages] = useState([
    {
      id: 'welcome',
      role: 'assistant',
      content: 'Hello! I am your personal finance RAG assistant powered by Gemini 3 Flash. I analyze your account balances, monthly budgets, upcoming bills, goals, and uploaded PDF documents/notes to provide grounded recommendations. How can I assist you today?',
      citations: []
    }
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `err-${Date.now()}`,
          role: 'assistant',
          content: 'Sorry, I encountered an error communicating with the financial AI service.',
          citations: []
        }
      ]);
    } finally {
  ...
  return (
    <div style={{ height: 'calc(100vh - 4rem)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ marginBottom: '1rem' }}>
        <h1 style={{ fontSize: '1.75rem', marginBottom: '0.25rem' }}>AI Financial Assistant</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
          Grounded RAG retrieval combining structured SQL records with ChromaDB document & note embeddings.
        </p>
      </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', paddingRight: '0.5rem' }}>
            ...
            >
              <div 
                style={{ 
                  ...
                  height: '36px', 
                  borderRadius: '50%', 
                  background: msg.role === 'user' ? 'var(--accent-primary)' : 'linear-gradient(135deg, #10b981, #6366f1)', 
                  display: 'flex', 
                  alignItems: 'center', 
              ...
              <div 
                style={{ 
                  maxWidth: '75%', 
                  background: msg.role === 'user' ? 'rgba(99,102,241,0.2)' : 'rgba(0,0,0,0.3)', 
          ...
          {loading && (
            <div style={{ display: 'flex', gap: '0.85rem', marginBottom: '1.25rem' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #10b981, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Bot size={18} color="#fff" />
              </div>
              <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-color)', borderRadius: '14px', padding: '1rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                Gemini 3 Flash is fetching SQL snapshot + vector ChromaDB context...
              </div>
            </div>
          )}
